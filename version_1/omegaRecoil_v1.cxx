...
//cout<<" AFTER PRIMARY VERTEX "<<endl;
    //************************************************
...
//cout<<" GOOD PHOTON SELECTION   Gamma: "<<nGamma<<endl;
    //************************************************
...
//cout<<" AFTER ASSIGIN 4-MOMENTUM TO EACH PHOTON "<<endl;
    //************************************************
...
cout<<" ASSIGN 4-MOMENTUM TO EACH CHARGED TRACK "<<endl;
    //************************************************
...
cout<<" Build  pi0    nPi0: "<<nPi0<<endl;
    ...
    for(int j=0;j<nGood;j++){
        if(idparticle[j]==4) pPip.push_back(pAtrk[j]);
        if(idparticle[j]==5) pPim.push_back(pAtrk[j]); 
    } 
    int npip = pPip.size(); int npim = pPim.size();
    if(npip<1||npim<1) return StatusCode::SUCCESS;
    //***********************************
cout<<" FIND Pi+ and Pi-    nPip: "<<npip<<"  "<<"nPim: "<<npim<<endl;
    ...
    for(int i0=0;i0<nPi0;i0++){
    	for(int i1=0;i1<npip;i1++){
    		for(int i2=0;i2<npim;i2++){
                HepLorentzVector omega = pGamma[gamma1_pi0[i0]]+pGamma[gamma2_pi0[i0]]+pPip[i1]+pPim[i2];
    			if(fabs(omega.m()-momega)<0.030){
    			pip_omega.push_back(i1);
    			pim_omega.push_back(i2);
...
cout<<" ipip_omega: "<<ipip_omega<<"  "<<" ipim_omega: "<<ipim_omega<<"  "<<" igamma1_omega: "<<igamma1_omega<<"  "<<" igamma2_omega: "<<igamma2_omega<<endl;            
    //**************save in TClonsArray***************
