...
StatusCode omegaRecoil::initialize(){
	...
	Omega = new TClonesArray("TLorentzVector");
	//*********** tree structure ******************
	if(m_readSig==1){
		...
		TreeAna->Branch("Omega","TClonesArray", &Omega, 256000,0);
...
StatusCode omegaRecoil::execute() {
	...
	for(int i0=0;i0<nPi0;i0++){
		for(int i1=0;i1<n_vtxpippim;i1++){
			HepLorentzVector pomega = pGamma[gamma1_pi0[i0]]+pGamma[gamma2_pi0[i0]]+pAtrk[viPip[i1]]+pAtrk[viPim[i1]];
			if(fabs(pomega.m()-momega)<0.030){
				...
				deta_omega.push_back(pow(fabs(pomega.m()-momega),2));
			}
		}
	}
	...
	HepLorentzVector omegaptrk = pGamma[igamma1_omega]+pGamma[igamma2_omega]+pAtrk[ipip_omega]+pAtrk[ipim_omega];
	TLorentzVector omega(0,0,0,0);
	omega.SetPxPyPzE(omegaptrk.px(),omegaptrk.py(),omegaptrk.pz(),omegaptrk.e());
	new((*Omega)[0])TLorentzVector(omega);
	double m_omega;
	m_omega = omega.M();
	TreeAna->Fill();

	cout<<"nelectronp:"<<n0<<" "<<"nelectronm:"<<n1<<" "<<"nmounp:"<<n2<<" "<<"nmounm:"<<n3<<" "<<"npip:"<<n4<<" "
		<<"npim:"<<n5<<" "<<"nkp:"<<n6<<" "<<"nkm:"<<n7<<" "<<"nprotonp:"<<n8<<" "<<"nprotonm:"<<n9<<" "<<"nGamma_n10:"<<n10<<"  "<<"nGamma: "<<nGamma<<endl;
...
void omegaRecoil::Vtxfit(SmartDataPtr<EvtRecTrackCol> evtRecTrkCol,Vint iPip,Int_t npip,Vint iPim,Int_t npim,Vint *viPip,Vint *viPim){
	...
	for (int i=0; i<npip; i++) {
		for (int j=0;j<npim;j++) {
			...
			wvpipTrk = WTrackParameter(xmass[2], PipTrk->getZHelix(), PipTrk->getZError());
			wvpimTrk = WTrackParameter(xmass[2], PimTrk->getZHelix(), PimTrk->getZError());
...
//**********************************
void omegaRecoil::Reconstruct_2gamma(SmartDataPtr<EvtRecTrackCol> evtRecTrkCol,Vint iGamma,Int_t nGamma,Vp4 pGamma,double Rec_m,Vint *gamma1_pi0,Vint *gamma2_pi0){
	...
	for(int i1=0;i1<nGamma-1;i1++){
		...
		for(int i2=i1+1;i2<nGamma;i2++){
			...
			(*gamma1_pi0).push_back(i1);
			(*gamma2_pi0).push_back(i2);
